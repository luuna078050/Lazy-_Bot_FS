"""LazyBot FS — Fast Scalper 3m/short-pump runner with mixed PAPER/LIVE slots."""
...
MAX_HOLD=180; PUMP_HOLD=20; FEE_PCT=float(os.getenv('FAST_SCALPER_ROUND_TRIP_FEE_PCT','0.20'))/100
...
def load_state():
    ...
    return {'mode':'MIXED' if LIVE_PAIRS and PAPER_PAIRS else ('LIVE' if LIVE_PAIRS else 'PAPER'),'capital':CAPITAL,'free_capital':CAPITAL,'realized_pnl':0.0,'positions':{},'orders':{},'trades':[],'started_at':time.time(),'error':None}
...
def close(s,symbol,price,reason,now):
    ...
    del s['positions'][symbol]; s['orders'].pop(symbol,None)
