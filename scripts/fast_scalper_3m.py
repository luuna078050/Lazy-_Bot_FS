...
def read_cfg():
    ...
    return float(os.getenv("FAST_SCALPER_CAPITAL_USDT","100")),tuple(x.strip().upper().replace("-","/") for x in os.getenv("FAST_SCALPER_PAIRS","DGB/USDT,ZRO/USDT,TUT/USDT").split(",") if x.strip()),tuple(float(x) for x in os.getenv("FAST_SCALPER_ALLOCATIONS","30,30,40").split(",")),float(os.getenv("FAST_SCALPER_MIN_PROFIT_USDT","0.20")),float(os.getenv("FAST_SCALPER_TARGET_PROFIT_USDT","0.30")),os.getenv("FAST_SCALPER_STOP_LOSS_ENABLED","true").lower()=="true",float(os.getenv("FAST_SCALPER_STOP_LOSS_PCT","0.50"))
CAPITAL,SYMBOLS,ALLOCS,MIN_PROFIT,TARGET_PROFIT,SL_ENABLED,SL_PCT=read_cfg(); MAX_HOLD=180; FEE_PCT=float(os.getenv("FAST_SCALPER_ROUND_TRIP_FEE_PCT","0.20"))/100
...
STATE=Path(os.getenv("FAST_SCALPER_STATE_FILE","fast_scalper_3m_state.json")); CFG=FastScalperConfig(CAPITAL,SYMBOLS,ALLOCS,"3m",MAX_HOLD,MIN_PROFIT,TARGET_PROFIT,SL_ENABLED,SL_PCT); validate(CFG); EX=gateway("binance")
def load_state():
    if STATE.exists():
...
def live_order(side,symbol,amount): return EX.create_market_order(symbol,side,amount,live=True) if LIVE else {"dry_run":True,"side":side,"symbol":symbol,"amount":amount}
def close(s,symbol,price,reason,now):
    ...
    if LIVE:
        ...
        order=live_order("sell",symbol,sell_amount); price=float(order.get("average") or order.get("price") or price); amount=float(order.get("filled") or sell_amount)
    profit=pnl({**pos,"amount":amount},price); s["realized_pnl"]+=profit; s["free_capital"]+=float(pos["capital"])+profit; s["trades"].append({"symbol":symbol,"entry":pos["entry"],"exit":price,"capital":pos["capital"],"amount":amount,"pnl":profit,"hold_sec":now-float(pos["opened"]),"reason":reason,"ts":now}); del s["positions"][symbol]
def main():
    state=load_state(); pulse=RealtimePulse(SYMBOLS,window_seconds=12); pulse.start(); rec=recommended_profit(CAPITAL/len(SYMBOLS),FEE_PCT*100); print(json.dumps({"mode":"LIVE" if LIVE else "PAPER","exchange":"binance","capital":CAPITAL,"pairs":SYMBOLS,"allocations_pct":ALLOCS,"timeframe":"3m","trade_budget_sec":MAX_HOLD,"minimum_profit_usdt":MIN_PROFIT,"target_profit_usdt":TARGET_PROFIT,"recommended":rec,"stop_loss":SL_ENABLED,"pulse":"1s","live_orders":LIVE},ensure_ascii=False))
    try:
        while True:
            ...
            for idx,symbol in enumerate(SYMBOLS):
                raw=snaps.get(symbol.replace('/','')) or snaps.get(symbol)
                if not raw or symbol in state["positions"] or raw.get("state") not in {"IGNITION","EARLY_ROCKET"}:continue
                ...
                try:
                    ...
                    order=live_order("buy",symbol,amount); actual_price=float(order.get("average") or order.get("price") or price); actual_amount=float(order.get("filled") or amount)
                    state["free_capital"]-=capital; state["positions"][symbol]={"entry":actual_price,"capital":capital,"amount":actual_amount,"opened":now,"score":float(raw["score"]),"state":raw["state"],"allocation_pct":ALLOCS[idx]}; print(json.dumps({"event":"ENTRY","mode":"LIVE" if LIVE else "PAPER","symbol":symbol,"capital":capital,"price":actual_price,"amount":actual_amount,"score":raw["score"],"state":raw["state"]},ensure_ascii=False))
    ...
    except KeyboardInterrupt:pulse.stop();save(state);print("Stopped; state saved")
