...
MAX_HOLD=180; PUMP_HOLD=20
FEE_PCT=float(os.getenv('FAST_SCALPER_ROUND_TRIP_FEE_PCT','0.20'))/100
ROTATION_MIN_HOLD_SEC=float(os.getenv('FAST_SCALPER_ROTATION_MIN_HOLD_SEC','20')); ROTATION_SCORE_GAP=float(os.getenv('FAST_SCALPER_ROTATION_SCORE_GAP','0.18')); ROTATION_MIN_ALT_SCORE=float(os.getenv('FAST_SCALPER_ROTATION_MIN_ALT_SCORE','0.58')); STALE_CHECK_SEC=float(os.getenv('FAST_SCALPER_STALE_CHECK_SEC','60')); STALE_MIN_SCORE=float(os.getenv('FAST_SCALPER_STALE_MIN_SCORE','0.40')); ROTATION_COOLDOWN_SEC=float(os.getenv('FAST_SCALPER_ROTATION_COOLDOWN_SEC','10'))
LIVE_PAIRS={x.strip().upper().replace('-','/') for x in os.getenv('FAST_SCALPER_LIVE_PAIRS','').split(',') if x.strip()}; PAPER_PAIRS={x.strip().upper().replace('-','/') for x in os.getenv('FAST_SCALPER_PAPER_PAIRS','').split(',') if x.strip()}; USE_AMOUNTS=[x.strip().lower()=='true' for x in os.getenv('FAST_SCALPER_USE_ORDER_AMOUNTS','').split(',')]; ORDER_AMOUNTS=[float(x or 0) for x in os.getenv('FAST_SCALPER_ORDER_AMOUNTS','').split(',') if x.strip()]
...
def slot_budget(idx):
    allocated=CAPITAL*ALLOCS[idx]/100; use=USE_AMOUNTS[idx] if idx<len(USE_AMOUNTS) else False; explicit=ORDER_AMOUNTS[idx] if idx<len(ORDER_AMOUNTS) else 0
    return min(allocated, explicit) if use and explicit>0 else allocated

def load_state():
    if STATE.exists():
        try:
            s=json.loads(STATE.read_text()); s.setdefault('cooldowns',{}); s.setdefault('rotation_count',0); s.setdefault('rotation_pnl',0.0)
            if s.get('profit_policy') != 'FIXED_BOT_CAPITAL_ACCUMULATE_PROFIT':
                # Migration from the old reinvesting behavior. Preserve open
                # positions, but rebuild free trading capital from the fixed
                # bot budget instead of carrying old realized profits forward.
                committed=sum(float(p.get('capital',0)) for p in (s.get('positions') or {}).values())
                s['free_capital']=max(0.0, CAPITAL-committed)
                s['accumulated_profit_usdt']=float(s.get('realized_pnl',0.0) or 0.0)
                s['profit_policy']='FIXED_BOT_CAPITAL_ACCUMULATE_PROFIT'
            else: s.setdefault('accumulated_profit_usdt',0.0)
            return s
        except Exception: pass
    return {'mode':'MIXED' if LIVE_PAIRS and PAPER_PAIRS else ('LIVE' if LIVE_PAIRS else 'PAPER'),'capital':CAPITAL,'free_capital':CAPITAL,'realized_pnl':0.0,'accumulated_profit_usdt':0.0,'profit_policy':'FIXED_BOT_CAPITAL_ACCUMULATE_PROFIT','positions':{},'orders':{},'trades':[],'cooldowns':{},'rotation_count':0,'rotation_pnl':0.0,'started_at':time.time(),'error':None}

def save(s):
    STATE.parent.mkdir(parents=True,exist_ok=True); STATE.write_text(json.dumps(s,indent=2,ensure_ascii=False))
...
def close(s,symbol,price,reason,now):
    pos=s['positions'][symbol]; amount=float(pos['amount']); live=is_live_symbol(symbol)
    if live:
        sell_amount=amount*(1-FEE_PCT/2); EX.load_markets(); sell_amount=float(EX.exchange.amount_to_precision(symbol,sell_amount))
        if sell_amount<=0:return
        order=EX.create_market_order(symbol,'sell',sell_amount,live=True); price=float(order.get('average') or order.get('price') or price); amount=float(order.get('filled') or sell_amount)
    ...
    profit=pnl({**pos,'amount':amount},price); s['realized_pnl']+=profit
    # Fixed bot capital: return only principal to the trading pool. Profit is
    # accumulated separately and is deliberately unavailable for new entries.
    s['free_capital']+=float(pos['capital']); s['accumulated_profit_usdt']=float(s.get('accumulated_profit_usdt',0.0))+profit; s['profit_policy']='FIXED_BOT_CAPITAL_ACCUMULATE_PROFIT'
    is_rotation=reason.startswith('CAPITAL_ROTATION')
    if is_rotation:
        s['rotation_count']=int(s.get('rotation_count',0))+1; s['rotation_pnl']=float(s.get('rotation_pnl',0))+profit; s.setdefault('cooldowns',{})[symbol]=now+ROTATION_COOLDOWN_SEC
    s['trades'].append({'symbol':symbol,'mode':'LIVE' if live else 'PAPER','entry':pos['entry'],'exit':price,'capital':pos['capital'],'amount':amount,'pnl':profit,'hold_sec':now-float(pos['opened']),'reason':reason,'signal':pos.get('signal'),'fills':pos.get('fills',[]),'ts':now}); del s['positions'][symbol]; s['orders'].pop(symbol,None)
...
def best_alternative(snaps,held):
    ...
    for symbol in SYMBOLS:
        ...
        if not raw or raw.get('state') not in {'IGNITION','EARLY_ROCKET'} or float(raw.get('score',0))<ROTATION_MIN_ALT_SCORE:continue
...
if LIVE_PAIRS:
    try:
        EX.load_markets(); bal=EX.fetch_balance(); free=float((bal.get('free') or {}).get('USDT') or 0); live_cap=sum(slot_budget(i) for i,sym in enumerate(SYMBOLS) if is_live_symbol(sym))
...
try:
    while not STOP_REQUESTED:
        ...
        for idx,symbol in enumerate(SYMBOLS):
            ...
            try:
                if live:
                    ...
                    order=EX.create_market_order(symbol,'buy',amount,live=True); actual_price=float(order.get('average') or order.get('price') or price); actual_amount=float(order.get('filled') or amount); fills=order.get('trades',[]) or []
                    ...
                    actual_price=price; actual_amount=amount; fills=[]; s['orders'][symbol]={'symbol':symbol,'side':'BUY','requested_amount':amount,'filled_amount':amount,'remaining_amount':0.0,'status':'PAPER_FILLED','order_id':None,'fills':[]}
                s['free_capital']-=budget; s['positions'][symbol]={'entry':actual_price,'capital':budget,'amount':actual_amount,'opened':now,'score':score,'state':raw['state'],'signal':signal_type,'hold_sec':PUMP_HOLD if signal_type=='PUMP' else MAX_HOLD,'allocation_pct':ALLOCS[idx],'fills':fills,'mode':'LIVE' if live else 'PAPER'}
        ...
        s['updated_at']=now; s['capital_efficiency']={'free_capital':s.get('free_capital',0),'rotation_count':s.get('rotation_count',0),'rotation_pnl':s.get('rotation_pnl',0.0),'accumulated_profit_usdt':s.get('accumulated_profit_usdt',0.0),'profit_policy':'FIXED_BOT_CAPITAL_ACCUMULATE_PROFIT'}; save(s); time.sleep(1)
finally:
    pulse.stop();save(s)
