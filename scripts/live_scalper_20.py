...
no withdrawals. Radar: top 10 from a liquid universe; up to five slots 10/10/20/20/30%,
leaving 10% reserve. Entry requires MTF + order-book confirmation; exits are TP/SL.
"""
...
load_dotenv()
QUOTE=os.getenv("QUOTE_CURRENCY","USDT").upper();CAPITAL=float(os.getenv("TEST_CAPITAL_USDT","20"));SCAN_LIMIT=int(os.getenv("SCAN_UNIVERSE","30"));RADAR_LIMIT=int(os.getenv("RADAR_LIMIT","10"));MAX_POSITIONS=min(5,int(os.getenv("MAX_POSITIONS","5")));TP=float(os.getenv("TAKE_PROFIT_PCT","0.6"))/100;SL=float(os.getenv("STOP_LOSS_PCT","0.3"))/100;MIN_SCORE=float(os.getenv("ENTRY_SCORE","0.30"));POLL=int(os.getenv("POLL_SECONDS","20"));STATE=Path(os.getenv("SCALPER_STATE","scalper_state.json"))
...
def _tf_data(ex,symbol):
    out={tf:_series(ex,symbol,tf) for tf in ("1m","3m","5m","15m","30m","1h","4h")}
...
def _allocation(score):return 30 if score>=.55 else 20 if score>=.40 else 10
...
def run_once(ex,state):
    ...
    for symbol,pos in list(state["positions"].items()):
        try:
            ...
            else:pos.update({"last":last,"pnl_pct":pnl})
        except Exception:continue
