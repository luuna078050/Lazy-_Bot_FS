...
exchange account balance. Automatic allocation is intentionally frozen during
the initial validation period. When enabled later, the strategy may dynamically
size positions from signal quality, but a hard per-position cap remains 40% of
bot balance unless the user explicitly changes that policy after validation.
"""
...
BOT_BALANCE=float(os.getenv("BOT_ACCOUNT_BALANCE","20"))
ACCOUNT_BALANCE_VIEW=float(os.getenv("ACCOUNT_BALANCE_VIEW","0"))
SCAN_LIMIT=int(os.getenv("SCAN_UNIVERSE","30"));RADAR_LIMIT=int(os.getenv("RADAR_LIMIT","10"));MAX_POSITIONS=min(5,int(os.getenv("MAX_POSITIONS","5")))
AUTO_ALLOCATION_ENABLED=os.getenv("AUTO_ALLOCATION_ENABLED","false").lower()=="true"
AUTO_ALLOCATION_MAX_PCT=min(40.0,float(os.getenv("AUTO_ALLOCATION_MAX_PCT","40")))
TEST_DAYS=int(os.getenv("ALLOCATION_VALIDATION_DAYS","14"));TEST_START=float(os.getenv("ALLOCATION_VALIDATION_START","0"))
...
def _load_state():
    ...
    return {"positions":{},"realized_pnl":0.0,"day":time.strftime("%Y-%m-%d"),"trades":[],"allocation_validation_start":time.time()}
...
def _allocation(score,validation_days):
    # Automatic sizing is disabled during the validation window.
    if not AUTO_ALLOCATION_ENABLED or validation_days<TEST_DAYS:return None
...
def run_once(ex,state):
    ...
    state["allocation_policy"]={"auto_enabled":AUTO_ALLOCATION_ENABLED,"validation_days":round(validation_days,2),"required_days":TEST_DAYS,"max_auto_position_pct":AUTO_ALLOCATION_MAX_PCT,"status":"LOCKED" if not AUTO_ALLOCATION_ENABLED or validation_days<TEST_DAYS else "ENABLED"}
    ...
    # During validation, no automatic allocation. Existing/manual positions are managed only.
    if not AUTO_ALLOCATION_ENABLED or validation_days<TEST_DAYS:
    ...
    used=set(state["positions"]);used_pct=sum(float(p.get("allocation_pct",0)) for p in state["positions"].values())
    free=max(0,MAX_POSITIONS-len(used))
    ...
    _save_state(state);return {"live":_live(),"bot_balance":BOT_BALANCE,"account_balance_view":ACCOUNT_BALANCE_VIEW,"allocated_pct":used_pct,"free":BOT_BALANCE*(1-used_pct/100),"allocation_policy":state["allocation_policy"],"radar":now,"positions":state["positions"],"realized_pnl":state["realized_pnl"]}
def main():
    ex=gateway(os.getenv("EXCHANGE","binance"));state=_load_state();print(f"LazyBot FS | {'LIVE' if _live() else 'PAPER'} | bot_balance={BOT_BALANCE} {QUOTE} | auto-allocation={'ON' if AUTO_ALLOCATION_ENABLED else 'LOCKED'}")
    while True:
