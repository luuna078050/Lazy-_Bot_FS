...
def install() -> None:
    ...
    def ensure_state(self):
        if not hasattr(self, "wall_history"): self.wall_history = defaultdict(lambda: deque(maxlen=240))
        if not hasattr(self, "wall_latest"): self.wall_latest = {}
        if not hasattr(self, "_current_scoring_symbol"): self._current_scoring_symbol = ""
        if not hasattr(self, "_current_scoring_price"): self._current_scoring_price = 0.0

    def build_url(self):
        ensure_state(self)
        url = original_build_url(self)
        try:
            parts = urlsplit(url); marker = "/stream?streams="
            if marker not in parts.path: return url
            base = parts.path.split(marker, 1)[1].split("/")
            existing = set(base)
            for s in self._top_symbols():
                stream = f"{s.lower()}@depth20@100ms"
                if stream not in existing: base.append(stream)
    ...
    def record_depth(self, data):
        ensure_state(self)
        symbol = str(data.get("s", "")).upper(); bids = data.get("b") or data.get("bids") or []; asks = data.get("a") or data.get("asks") or []
        if not symbol or not bids or not asks: return
        try:
            ...
            if not bid_levels or not ask_levels: return
            best_bid = max(p for p, _ in bid_levels); best_ask = min(p for p, _ in ask_levels); mid = (best_bid + best_ask) / 2
    ...
    def wall_metrics(self, symbol, price):
        ensure_state(self); now = time.time()
        with self.lock: hist = [x for x in self.wall_history.get(symbol, ()) if now - x["ts"] <= 180]
        empty = {"wall_score": 0.5, "wall_direction": "neutral", "support_wall_price": None, "resistance_wall_price": None,
                 "support_shift_3m_pct": 0.0, "resistance_shift_3m_pct": 0.0, "ask_wall_absorption": 0.0,
                 "bid_wall_strength": 0.0, "breakout_confirmed": False, "retest_confirmed": False, "wall_persistence": 0.0}
        if not hist or price <= 0: return empty
        first, last = hist[0], hist[-1]
        def pct(a, b): return ((b / a) - 1.0) * 100 if a else 0.0
        support_shift = pct(first["bid_wall_price"], last["bid_wall_price"]); resistance_shift = pct(first["ask_wall_price"], last["ask_wall_price"])
        prev_ask_max = max((x["ask_wall_quote"] for x in hist[:-1]), default=last["ask_wall_quote"]); prev_bid_max = max((x["bid_wall_quote"] for x in hist[:-1]), default=last["bid_wall_quote"])
        ...
        support_up = max(0.0, min(1.0, support_shift / 0.5)); resistance_pressure = max(0.0, min(1.0, -resistance_shift / 0.5))
        ...
        score = max(0.0, min(1.0, raw)); direction = "bullish" if score >= 0.62 else "bearish" if score <= 0.38 else "neutral"
        return {"wall_score": round(score,4), "wall_direction": direction, "support_wall_price": last["bid_wall_price"], "resistance_wall_price": last["ask_wall_price"],
                "support_shift_3m_pct": round(support_shift,5), "resistance_shift_3m_pct": round(resistance_shift,5), "ask_wall_absorption": round(ask_absorption,4),
                "bid_wall_strength": round(bid_strength,4), "breakout_confirmed": breakout, "retest_confirmed": retest, "wall_persistence": round(min(1.0, len(hist)/180.0),4)}

    def timeframe_metrics(self, symbol, price):
        ensure_state(self); self._current_scoring_symbol = symbol; self._current_scoring_price = price
        return original_timeframe_metrics(self, symbol, price)

    def opportunity_score(self, hot, tf, pulse, liquidity, pct24):
        ensure_state(self); base_score, components = original_opportunity_score(self, hot, tf, pulse, liquidity, pct24)
        wall = self.wall_metrics(self._current_scoring_symbol, self._current_scoring_price); components = dict(components); components.update(wall)
        score = 100.0 * (0.88 * base_score / 100.0 + 0.12 * wall["wall_score"])
        return round(self._clamp(score, 0, 100), 2), components

    MarketRadar.__init__ = init; MarketRadar._build_url = build_url; MarketRadar._on_message = on_message
    MarketRadar._timeframe_metrics = timeframe_metrics; MarketRadar._wall_metrics = wall_metrics; MarketRadar._opportunity_score = opportunity_score

    def wall_snapshot(self, symbol: str, price: float | None = None):
        ensure_state(self); s = str(symbol).upper().replace("/", "")
    ...
    # MarketRadar is started during module import before main.py installs patches.
    # Close the old socket so its run loop rebuilds the URL with depth streams.
    try:
        from .market_radar import RADAR
        ensure_state(RADAR)
        if getattr(RADAR, "_ws", None): RADAR._ws.close()
    except Exception:
        pass
