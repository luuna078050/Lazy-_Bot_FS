...
app=FastAPI(title='LazyBot FS',version='1.0.0')
def paper_config():
    risk=load_settings()
...
def fast_scalper_config():
    capital=float(os.getenv('FAST_SCALPER_CAPITAL_USDT','100')); pairs=tuple(x.strip() for x in os.getenv('FAST_SCALPER_PAIRS','DGB/USDT,ZRO/USDT,TUT/USDT').split(',') if x.strip()); alloc=tuple(float(x) for x in os.getenv('FAST_SCALPER_ALLOCATIONS','30,30,40').split(',')); minp=float(os.getenv('FAST_SCALPER_MIN_PROFIT_USDT','0.20')); target=float(os.getenv('FAST_SCALPER_TARGET_PROFIT_USDT','0.30')); cfg=FastScalperConfig(capital,pairs,alloc,'3m',180,minp,target,os.getenv('FAST_SCALPER_STOP_LOSS_ENABLED','true').lower()=='true',float(os.getenv('FAST_SCALPER_STOP_LOSS_PCT','0.50'))); validate_fast(cfg); return {'capital_usdt':capital,'pairs':pairs,'allocations_pct':alloc,'timeframe':'3m','max_trade_seconds':180,'min_profit_usdt':minp,'target_profit_usdt':target,'stop_loss_enabled':cfg.stop_loss_enabled,'stop_loss_pct':cfg.stop_loss_pct,'recommended_profit':recommended_profit(capital/len(pairs),float(os.getenv('FAST_SCALPER_ROUND_TRIP_FEE_PCT','0')))}
@app.get('/api/health')
