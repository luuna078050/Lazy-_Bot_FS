...
app = FastAPI(title="LazyBot FS", version="0.5.0")

def paper_config():
    return {"max_position_usdt": float(os.getenv("MAX_POSITION_USDT", 5)), "risk_per_trade_usdt": float(os.getenv("RISK_PER_TRADE_USDT", 1)), "daily_loss_limit_usdt": float(os.getenv("DAILY_LOSS_LIMIT_USDT", 3)), "leverage": int(os.getenv("LEVERAGE", 1)), "take_profit_pct": float(os.getenv("TAKE_PROFIT_PCT", 0.6)), "stop_loss_pct": float(os.getenv("STOP_LOSS_PCT", 0.3))}
...
def orderbook_analyze(exchange_id: str, symbol: str, limit: int = 50):
    """Fetch and score live depth. No order is placed."""
    try:
        book = gateway(exchange_id).fetch_order_book(symbol, max(5, min(limit, 500)))
        result = analyze_orderbook(symbol, book)
        result.update({"exchange": exchange_id, "symbol": symbol, "best_bid": book["bids"][0][0] if book.get("bids") else None, "best_ask": book["asks"][0][0] if book.get("asks") else None})
        return result
    except Exception as exc: raise HTTPException(status_code=400, detail=str(exc))
