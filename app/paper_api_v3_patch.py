...
def _remove(path):
    for route in list(core.app.router.routes):
        if getattr(route,'path',None)==path and getattr(route,'methods',None): core.app.router.routes.remove(route)

def install(app):
    for path in ('/api/paper/start','/api/paper/stop','/api/paper/emergency-stop','/api/paper/status'): _remove(path)
    @app.post('/api/paper/start')
    def paper_start_v7(payload:dict):
    ...
    def paper_stop_v7():return stop_paper()
    @app.post('/api/paper/emergency-stop')
    def paper_emergency_v7():return emergency_stop_paper()
    @app.get('/api/paper/status')
    def paper_status_v7():return snapshot()
