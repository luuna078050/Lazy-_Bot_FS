...
def decision(m: dict[str, Any]) -> dict[str, Any]:
    c3 = float(m.get("change_3m_pct", 0))
    vr = float(m.get("volume_ratio", 1))
    buy = float(m.get("buy_ratio", 0.5))
    pump = float(m.get("pump_score", 0))
    c24 = float(m.get("change_24h_pct", 0))
    signal = str(m.get("signal", "WAIT"))
    # The entry gate deliberately requires confirmation from several independent
    # layers. A 24h trend alone is NOT a scalping entry anymore.
    momentum = max(0.0, min(1.0, c3 / 0.60))
    volume = max(0.0, min(1.0, (vr - 1.0) / 2.5))
    flow = max(0.0, min(1.0, (buy - 0.50) / 0.20))
    ...
    entry_ok = signal != "WAIT" and signal != "FADE" and confirmations >= 4 and quality >= 58
    return {"quality": round(quality, 2), "confirmations": confirmations, "entry_ok": entry_ok}
...
def snapshot() -> dict[str, Any]:
    ...
    equity = float(s.get("account_balance_usdt", 0)) + unreal
    closed = s["trades"]
    ...
    s.update({
        ...
        "net_pnl": equity - float(s.get("initial_balance", 0)),
        "balance": float(s.get("free_usdt", 0)),
        "pnl": float(s.get("realized_pnl", 0)),
        "bot_balance_usdt": float(s.get("initial_balance", 0)),
        "available_bot_usdt": float(s.get("free_usdt", 0)),
...
def close(symbol: str, pos: dict[str, Any], last: float, reason: str) -> None:
    ...
    # Profit is booked separately and never becomes trading capital.
    STATE["profit_reserve_usdt"] = float(STATE.get("profit_reserve_usdt", 0)) + max(0.0, net)
    STATE["trades"].append({
        "symbol": symbol, "side": "SELL", "entry_price": entry, "exit_price": last,
        "allocated_usdt": cap, "gross_pnl": gross, "fee": fee, "net_pnl": net,
        "reason": reason, "opened_at": pos["opened_at"], "closed_at": now(),
        "signal": pos.get("signal"), "quality": pos.get("quality"),
        "confirmations": pos.get("confirmations"), "timeframe": "3m",
    })
...
def tick(symbol: str, allocation: float) -> None:
    ...
    with LOCK:
        ...
        if pos:
            ...
            move_pct = (last / float(pos["entry_price"]) - 1) * 100
            target_usdt = max(float(cfg["min_profit_usdt"]), float(cfg["target_profit_usdt"]))
            reason = None
            # Positive-after-fees target only. The old 0.14% target could be
            # negative after fees; that was a direct source of losing closes.
            if net >= target_usdt:
                reason = "TARGET_PROFIT"
            elif net > 0 and age >= float(pos["hold_seconds"]):
                reason = "TIME_EXIT_PROFIT"
            elif net >= 0 and str(m.get("signal")) == "FADE":
                reason = "REVERSAL_PROFIT"
            # No normal timeout or rotation may close an underwater trade.
            # Only a catastrophic adverse move is allowed to realize a loss.
            elif move_pct <= -1.20:
                reason = "CATASTROPHIC_STOP"
            if reason:
                close(symbol, pos, last, reason)
            else:
                base = symbol.split("/")[0]
            ...
            return
        if HALT_ENTRIES or STATE["free_usdt"] <= 0 or allocation <= 0 or time.time() < COOLDOWN.get(symbol, 0):
            return
        if allocation > STATE["free_usdt"] + 1e-9:
            return
        d = decision(m)
        if not d["entry_ok"]:
            return
        amount = allocation / last
        fills = [{"time": now(), "amount": amount, "price": last, "cost": allocation}]
        ...
        STATE["open_positions"][symbol] = {
            ...
            "hold_seconds": int(m.get("hold_seconds", 180) or 180), "quality": d["quality"],
            "confirmations": d["confirmations"], "fills": fills, "stage": "OPEN",
        }
        STATE["order_history"].append({**STATE["orders"][symbol], "price": last, "time": now()})
        STATE["orders"].pop(symbol, None)
...
def loop() -> None:
    while not STOP.is_set():
        try:
            with LOCK:
                cfg = dict(STATE["config"])
            for i, symbol in enumerate(cfg["pairs"]):
                allocation = float(cfg["initial_balance"]) * float(cfg["allocations"][i]) / 100
                tick(symbol, allocation)
...
def start_paper(config: dict[str, Any], gateway_unused=None):
    ...
    if not 1 <= len(pairs) <= 5:
        raise ValueError("Выберите от 1 до 5 пар.")
    if len(allocations) != len(pairs) or any(x <= 0 for x in allocations) or abs(sum(allocations) - 100) > 0.01:
        raise ValueError("Доли выбранных пар должны дать ровно 100%.")
    if capital <= 0:
        raise ValueError("Бюджет PAPER должен быть больше 0 USDT.")
    RADAR.start()
    STOP.clear(); HALT_ENTRIES = False; COOLDOWN.clear()
    with LOCK:
        STATE.update({
            "running": True, "mode": "paper", "started_at": now(), "stopped_at": None,
            ...
            "config": {
                "pairs": pairs, "allocations": allocations, "initial_balance": capital,
                "min_profit_usdt": float(config.get("min_profit_usdt", 0.05)),
                "target_profit_usdt": float(config.get("target_profit_usdt", 0.10)),
                "fee_pct": float(config.get("fee_pct", 0.10)),
                "timeframe": "3m", "risk_mode": "PROFIT_FIRST",
            },
        })
    THREAD = threading.Thread(target=loop, daemon=True, name="fast-scalper-profit-first-paper")
    THREAD.start()
    return snapshot()
...
def emergency_stop_paper(gateway_unused=None):
    ...
    with LOCK:
        for symbol, pos in list(STATE["open_positions"].items()):
            ...
            close(symbol, pos, last, "EMERGENCY_STOP")
