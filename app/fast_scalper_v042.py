...
RADAR_INTERVAL=20; ROTATE_SECONDS=60; MAX_POSITION_SECONDS=420
...
class Slots(BaseModel):
    slots:list[str]=Field(default_factory=list,max_length=MAX_SLOTS); profit_pct:float=Field(0,ge=0,le=80); reinvest:bool=False
    ...
    for base in BASES:
        try:
            async with httpx.AsyncClient(timeout=7,headers={'User-Agent':'FastScalper/1.0'}) as c:
                ...
                if r.status_code in (418,429,403): last=RuntimeError(f'HTTP {r.status_code} from {base}'); continue
...
async def build_ranking():
    ...
    for sym in UNIVERSE:
        ...
        except Exception: vol=ch=0
        candidates.append((sym,vol,abs(ch)))
    candidates.sort(key=lambda z:(z[1],z[2]),reverse=True); candidates=[x[0] for x in candidates[:12]]
    async def one(sym):
        t=by.get(sym,{})
    ...
    rows=[x for x in await asyncio.gather(*(one(s) for s in candidates)) if x]
    rows.sort(key=lambda x:(x['score'],x['volume']),reverse=True); return rows[:15]

async def radar(force=False):
    if not force and S['last_radar'] and time.time()-S['last_radar']<RADAR_INTERVAL:return
    try:
        S['ranking']=await build_ranking(); S['last_radar']=time.time(); S['error']=None
...
def open_position(slot,sym):
    ...
    S['orders'].insert(0,{'time':now(),'symbol':sym,'side':'BUY','status':'FILLED','price':ep,'slot':slot,'score':score})

async def manage_positions():
    if not S['positions']: return
    try:
        tickers=await get_json('/api/v3/ticker/price'); latest={x.get('symbol'):float(x.get('price')) for x in tickers if isinstance(x,dict) and x.get('symbol')}
...
@app.post('/api/reset')
async def reset():
    S.update({'running':False,'account':START_ACCOUNT,'bot':START_BOT,'free':START_BOT,'realized':0.0,'session_realized':0.0,'session_trades':0,'session_started':None,'positions':[],'closed':[],'orders':[],'ranking':[],'slots':[None]*MAX_SLOTS,'profit':0.0,'reinvest':False,'cycle':0,'started':None,'last_radar':0.0,'error':None}); await radar(True); return await state()
@app.post('/api/slots')
...
HTML='''<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>Fast Scalper</title><style>*{box-sizing:border-box}body{margin:0;background:#080e1b;color:#eef3ff;font-family:system-ui}.w{max-width:900px;margin:auto;padding:14px}.title{font-size:30px;font-weight:900}.sub,.muted{color:#8b97ae}.card{background:#121a2c;border:1px solid #293650;border-radius:16px;padding:14px;margin:10px 0}.stats{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}.stat{background:#0d1425;border-radius:10px;padding:10px}.v{font-size:19px;font-weight:850}.row{display:flex;gap:8px;flex-wrap:wrap}.input{background:#0b1322;color:#fff;border:1px solid #30405f;border-radius:9px;padding:10px;flex:1;min-width:120px}.btn{border:0;border-radius:10px;padding:11px 15px;color:#fff;font-weight:850;background:#273650}.on{background:#078b53}.stop{background:#a72e3f}.grid6{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.slot{background:#0d1425;border:1px solid #293753;border-radius:10px;padding:9px}.rank{display:grid;grid-template-columns:26px 1fr 70px 60px 52px;gap:6px;align-items:center;padding:8px;border-bottom:1px solid #24314a}.badge{border-radius:7px;padding:3px 5px;text-align:center;background:#08764e}.good{color:#40df8b}.bad{color:#ff6576}@media(max-width:650px){.stats{grid-template-columns:repeat(2,1fr)}.grid6{grid-template-columns:repeat(2,1fr)}}details summary{cursor:pointer;font-weight:850}</style></head><body><div class="w"><div class="title">⚡ Fast Scalper</div><div class="sub">v0.4.2 REPAIR · Multi-TF 1m · 3m · 5m · 15m · 30m · Trading TF: 3m · PAPER</div><div class="card"><div class="stats"><div class="stat">Account Balance<div class="v" id="a">—</div></div><div class="stat">Realized PnL<div class="v" id="r">—</div></div><div class="stat">Session PnL<div class="v" id="sr">—</div></div><div class="stat">Bot Balance<div class="v" id="b">—</div></div></div></div><div class="card"><h3>Paper controls</h3><div class="row"><input id="p" class="input" type="number" step=".01" value="3.5" placeholder="Profit %"><button class="btn on" onclick="start()">PAPER ON</button><button class="btn stop" onclick="stop()">PAPER OFF</button><button class="btn stop" onclick="emergency()">EMERGENCY</button><button class="btn" onclick="resetApp()">RESET</button></div><div class="muted">Profit is a target. Rotation: 60s. Timeout: 420s. STOP blocks new entries but never freezes existing positions.</div></div><div class="card"><h3>Capital</h3><div class="row"><input id="amt" class="input" type="number" step=".01" placeholder="USDT"><button class="btn" onclick="alloc()">SET BOT</button><button class="btn" onclick="withdraw()">WITHDRAW</button></div><div class="muted" id="capital">—</div></div><div class="card"><h3>Slots · TOP-6</h3><div id="slots" class="grid6">—</div></div><div class="card"><h3>Radar · TOP-15</h3><div id="rank">—</div></div><div class="card"><h3>Open Positions</h3><div id="pos">—</div></div><div class="card"><h3>Closed Trades — latest 8</h3><div id="closed">—</div></div><div class="card"><details open><summary>Status / diagnostics</summary><div id="status" class="muted" style="margin-top:8px">—</div></details></div></div><script>async function api(url,opts){const r=await fetch(url,opts);const j=await r.json();if(!r.ok){alert(j.detail||'Error');throw Error(j.detail||'Error')}return j}function esc(x){return String(x).replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}async function refresh(){try{const j=await api('/api/state');a.textContent=(j.account??0).toFixed(4);r.textContent=(j.realized??0).toFixed(4);sr.textContent=(j.session_realized??0).toFixed(4);b.textContent=(j.bot_balance??0).toFixed(4);capital.textContent='Account free: '+(j.account_free??0).toFixed(4)+' · Bot free: '+(j.free??0).toFixed(4)+' · Total equity: '+(j.total_equity??0).toFixed(4)+' · Withdraw available: '+(j.withdraw_available??0).toFixed(4);status.textContent=(j.running?'ENGINE ON':'ENGINE OFF')+' · cycle '+j.cycle+' · positions '+j.positions.length+' · trades '+j.session_trades+' · radar '+(j.radar_age??0)+'s ago · '+(j.source||'market source pending')+(j.error?' · ERROR: '+j.error:'');slots.innerHTML=j.slots.map((x,i)=>x?'<div class="slot"><b>#'+(i+1)+' '+esc(x.symbol)+'</b><br><span class="muted">'+esc(x.tf)+' · '+(x.auto?'AUTO':'MANUAL')+'</span></div>':'<div class="slot"><span class="muted">#'+(i+1)+' empty</span></div>').join('');rank.innerHTML=(j.ranking||[]).map((x,i)=>'<div class="rank"><b>'+(i+1)+'</b><b>'+esc(x.symbol)+'</b><span>'+Number(x.score).toFixed(2)+'</span><span class='+(x.change>=0?'good':'bad')+'>'+Number(x.change).toFixed(2)+'%</span><span class="badge">'+esc(x.signal)+'</span></div>').join('')||'<span class="muted">No radar data yet</span>';pos.innerHTML=j.positions.map(x=>'<div style="padding:8px;border-bottom:1px solid #24314a"><b>'+esc(x.symbol)+'</b> · slot '+(x.slot+1)+' · stake '+Number(x.stake).toFixed(4)+' · age '+Math.floor(Date.now()/1000-x.opened)+'s · live '+(((x.current/x.entry-1)*100).toFixed(3))+'%</div>').join('')||'<span class="muted">No open positions</span>';closed.innerHTML=j.closed.slice(0,8).map(x=>'<div style="padding:7px;border-bottom:1px solid #24314a"><b>'+esc(x.symbol)+'</b> · '+esc(x.reason)+' · <span class='+(x.pnl>=0?'good':'bad')+'>'+Number(x.pnl).toFixed(4)+' USDT</span></div>').join('')||'<span class="muted">No closed trades</span>'}catch(e){status.textContent='API error: '+e.message}}async function start(){await api('/api/paper/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({profit_pct:Number(p.value)||0,reinvest:false})});refresh()}async function stop(){await api('/api/paper/stop',{method:'POST'});refresh()}async function emergency(){await api('/api/paper/emergency',{method:'POST'});refresh()}async function resetApp(){if(confirm('Reset paper session?')){await api('/api/reset',{method:'POST'});refresh()}}async function alloc(){await api('/api/strategy/allocate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({amount:Number(amt.value)})});refresh()}async function withdraw(){await api('/api/strategy/withdraw',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({amount:Number(amt.value)})});refresh()}refresh();setInterval(refresh,1000)</script></body></html>'''
