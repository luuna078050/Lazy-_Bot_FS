...
async def start(b:Start):
    ...
    S.update({'profit':b.profit_pct,'reinvest':b.reinvest,'running':True,'started':now(),'session_started':now(),'session_realized':0.0,'session_trades':0,'error':None});fill_auto_slots();return await state()
...
@app.post('/api/reset')
async def reset():
    S.update({'running':False,'account':START_ACCOUNT,'bot':0.0,'free':0.0,'realized':0.0,'session_realized':0.0,'session_trades':0,'session_started':None,'positions':[],'closed':[],'orders':[],'ranking':[],'slots':[None]*MAX_SLOTS,'profit':0.0,'reinvest':False,'cycle':0,'started':None,'last_radar':0.0,'error':None,'prices':{}});await radar(True);return await state()
@app.post('/api/slots')
