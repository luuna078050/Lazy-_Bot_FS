...
async def allocation(b:Allocation):
 if S['mode']!='BINANCE_TEST':raise HTTPException(403,'Bot Allocation is available in BINANCE_TEST mode')
 try:
...
async def mode(b:Mode):
 ...
 if m=='BINANCE_TEST':
  if not B.testnet:raise HTTPException(403,'BINANCE_TEST requires Binance Testnet')
  try:
   await B.ping();acc=await B.account();free_usdt=next((float(x['free']) for x in acc.get('balances',[]) if x.get('asset')=='USDT'),0.0);S['account']=free_usdt;S['bot']=0.0;S['free']=0.0;S['reserve']=free_usdt
  except Exception as e:raise HTTPException(400,f'Binance TEST mode failed: {type(e).__name__}: {e}')
 else:
  S['account']=START;S['bot']=0.0;S['free']=0.0;S['reserve']=0.0
