...
app.router.routes = [r for r in app.router.routes if not (getattr(r, 'path', None) == '/api/mode' and 'POST' in (getattr(r, 'methods', set()) or set()))]

@app.post('/api/mode')
async def mode_fixed(b: legacy.Mode):
    m = b.mode.upper()
    if m not in {'PAPER', 'BINANCE_TEST'}: raise HTTPException(403, 'Unsupported mode')
    if S['running'] or S['positions']: raise HTTPException(400, 'BOT OFF and no open positions required')
    ...
    if m == 'PAPER': S['account'] = START; S['reserve'] = START
    else: S['account'] = 0.0; S['reserve'] = 0.0
    return await legacy.state()

@app.post('/api/radar')
async def radar_endpoint():
    await legacy.radar(True); return await legacy.state()

@app.post('/api/withdraw')
async def withdraw_endpoint(b: dict):
    if S['running'] or S['positions']: raise HTTPException(400, 'BOT OFF and no open positions required')
    try: amount = float(b.get('amount', 0))
    except Exception: raise HTTPException(400, 'Enter withdrawal amount')
    if amount <= 0: raise HTTPException(400, 'Enter withdrawal amount')
    bot = float(S.get('bot', 0.0) or 0.0)
    if amount > bot + 1e-9: raise HTTPException(400, f'Withdraw amount exceeds Bot Balance ({bot:.4f} USDT)')
    S['bot'] = max(0.0, bot - amount); S['free'] = S['bot']; S['reserve'] = max(0.0, float(S.get('account', 0.0) or 0.0) - S['bot']); S['error'] = None
    return await legacy.state()
...
<div class="card"><div class="stats"><div class="stat">Account Balance<div class="v" id="account">—</div></div><div class="stat">Bot Balance<div class="v" id="bot">—</div></div><div class="stat">Reserve<div class="v" id="reserve">—</div></div><div class="stat">Session PnL<div class="v" id="sp">—</div></div></div><div class="row mode-row"><button type="button" class="btn mode" id="paperBtn">PAPER</button><button type="button" class="btn mode" id="testModeBtn">BINANCE TEST</button><button type="button" class="btn test" id="testBtn">TEST BINANCE</button></div><div id="msg" class="muted" style="margin-top:8px"></div></div>
...
<div class="card"><h2 class="section-title">Open Positions</h2><div id="tradeStatus" class="trade-status">BOT ON · TRADING ACTIVE</div><div id="pos" class="muted">No open positions</div></div>
<div class="card"><h2 class="section-title slots-title">Slots · TOP-10</h2><div class="grid6" id="slots"></div><div class="row" style="margin-top:10px"><button type="button" class="btn small-auto" id="topBtn">AUTO TOP-10</button></div><div class="help">Ручное изменение слота сохраняется автоматически.</div></div>
<div class="card"><details open><summary>Radar · TOP-20 · recommended pairs</summary><div class="radar-head"><span>#</span><span>PAIR</span><span>PRICE</span><span>SCORE</span><span></span></div><div id="radar"></div></details></div>
<div class="card"><h2 class="section-title">Closed Trades — latest 5</h2><div id="closed" class="muted">No closed trades</div></div></div>
<script>
let state={};let slotsDrawn=false;let modeBusy=false;const $=id=>document.getElementById(id);const profit=()=>Number($('profit').value)||0;const reinvest=()=>$('reinvest').checked;
...
function drawSlots(){const a=state.slots||[];$('slots').innerHTML=Array.from({length:10},(_,i)=>`<input type="text" class="slot" id="pair${i}" placeholder="USDT pair" value="${a[i]||''}">`).join('');for(let i=0;i<10;i++)$('pair'+i).addEventListener('change',saveSlots);slotsDrawn=true}
function syncSlots(){for(let i=0;i<10;i++){const e=$('pair'+i);if(e)e.value=(state.slots||[])[i]||''}}
async function saveSlots(){try{const a=Array.from({length:10},(_,i)=>$('pair'+i).value.trim()).filter(Boolean);state=await request('/api/slots','POST',{slots:a,profit_pct:profit(),reinvest:reinvest()});render();syncSlots();$('msg').textContent='Pairs saved'}catch(e){$('msg').textContent=e.message}}
function render(){const m=state.mode||'PAPER';$('paperBtn').className='btn mode'+(m==='PAPER'?' active':'');$('testModeBtn').className='btn mode'+(m==='BINANCE_TEST'?' test-active':'');$('account').textContent=num(state.account);$('bot').textContent=num(state.bot_balance);$('reserve').textContent=num(state.reserve);$('sp').textContent=num(state.session_realized);$('tim').textContent='SESSION '+clock(state.session_age)+' · 24H '+clock(state.day_age);const ts=$('tradeStatus');if(ts){const on=!!state.running;ts.textContent=on?'BOT ON · TRADING ACTIVE':(state.positions&&state.positions.length?'BOT OFF · CLOSING POSITIONS':'BOT OFF · TRADING STOPPED');ts.className='trade-status'+(on?'':' off')}const p=(state.positions||[]).slice().sort((a,b)=>(a.slot??999)-(b.slot??999));$('pos').innerHTML=p.length?p.map(x=>`<div class="line">${x.symbol} · ${num(x.stake)} USDT · ${num(x.current)}</div>`).join(''):'No open positions';$('closed').innerHTML=(state.closed||[]).slice(0,5).map(x=>`<div class="line">${x.symbol} · ${x.reason} · ${num(x.pnl)} USDT · ${num(x.exit)}</div>`).join('')||'No closed trades';$('radar').innerHTML=(state.ranking||[]).slice(0,20).map((x,i)=>`<div class="rank"><b>${i+1}</b><b>${x.symbol}</b><span>${num(x.price)}</span><span>${num(x.score)}</span><button type="button" class="btn add-radar" data-symbol="${x.symbol}">${(state.slots||[]).includes(x.symbol)?'IN SLOTS':'ADD PAIR'}</button></div>`).join('')||'Radar waiting for data';document.querySelectorAll('.add-radar').forEach(b=>b.addEventListener('click',()=>addRadarPair(b.dataset.symbol)))}
...
async function addRadarPair(symbol){try{const a=Array.from({length:10},(_,i)=>$('pair'+i).value.trim());if(a.includes(symbol)){$('msg').textContent=symbol+' already in slots';return}const i=a.findIndex(x=>!x);if(i<0){$('msg').textContent='Maximum 10 pairs';return}a[i]=symbol;state=await request('/api/slots','POST',{slots:a.filter(Boolean),profit_pct:profit(),reinvest:reinvest()});syncSlots();render();$('msg').textContent=symbol+' added to slot '+(i+1)}catch(e){$('msg').textContent=e.message}}
async function autoTop6Click(){try{state=await request('/api/slots/auto-top6','POST',{slots:[],profit_pct:profit(),reinvest:reinvest()});syncSlots();render();$('msg').textContent='AUTO TOP-10 applied'}catch(e){$('msg').textContent=e.message}}
