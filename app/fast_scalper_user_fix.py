...
async def radar_userfix(force=False):
    ...
    try:
        ...
        for x in rows:
            ...
            row = dict(x)
            row.update({'entry_allowed': usable, 'entry_score': round(score, 2), 'entry_confirmations': c,
                        'ema9_3m': round(ind['ema9'], 10), 'ema21_3m': round(ind['ema21'], 10),
                        'rsi14_3m': round(ind['rsi'], 2), 'indicator_tf': ind['tf'],
        ...
        _refresh_rotation_slots()
    except Exception as e:
        legacy.S['error'] = f'Radar: {type(e).__name__}: {e}'
        legacy.S['last_radar'] = time.time()


def _refresh_rotation_slots():
    ranked = []
    seen = set()
    ...
    used = {str(x).upper().replace('/', '') for x in old if x}
    ...
    for i in range(ROTATION_POOL):
        ...
        if candidate:
            previous = str(final_slots[i] or '').upper().replace('/', '')
...
async def open_userfix(i, symbol):
    if legacy.S.get('stop_requested') or not legacy.S.get('running'):
        return
    if i >= TRADE_SLOTS or not symbol or len(legacy.S.get('positions', [])) >= TRADE_SLOTS:
        return
    s = str(symbol).upper().replace('/', '')
    ...
    if not row or not row.get('entry_allowed'):
        return
        ...
        try:
            ...
            if legacy.S.get('running') and not legacy.S.get('stop_requested'):
                ...
                for i, s in enumerate(list(legacy.S.get('slots', []))[:ROTATION_POOL]):
                    ...
                    if len(legacy.S.get('positions', [])) >= TRADE_SLOTS:
                        break
...
# BOT OFF is a hard entry stop. It does not close existing positions.
...
# RESET restores the requested defaults and the full 20-pair rotation pool.
for r in list(legacy.app.router.routes):
...
async def reset_userfix():
    if legacy.S.get('running'):
        raise HTTPException(400, 'STOP the bot before RESET')
    for p in list(legacy.S.get('positions', [])):
        ok = await legacy.close(p, 'RESET')
        if ok is False:
            raise HTTPException(502, legacy.S.get('error') or 'Could not close position')
    legacy.S['slots'] = [None] * ROTATION_POOL
...
# Runtime defaults; user can still select any value below 0.33.
legacy.S['profit'] = DEFAULT_PROFIT
legacy.S['reinvest'] = True

html = legacy.HTML
html = html.replace('value="0.41"', 'value="0.33"').replace('value="0.30"', 'value="0.33"')
html = html.replace('Slots · TOP-20', 'Rotation Pool · TOP-20')
legacy.HTML = html

print('USER_FIX_ENABLED ROTATION_POOL=20 TRADE_SLOTS=10 TP=0.33 SOFT=90 HARD=300 BOT_OFF=HARD_ENTRY_STOP', flush=True)
