...
.run-mode{min-width:0}
.run-switch{cursor:pointer;touch-action:manipulation;-webkit-tap-highlight-color:transparent}
.run-switch:active{transform:scale(.98)}
...
(function(){
  ...
  function paint(mode,on,startedAt){
    ...
    const tick=()=>{
      if(!tm)return;
      if(!on){tm.textContent='00:00:00';return}
      const sec=Math.max(0,Math.floor((Date.now()-(Number(startedAt)||Date.now()))/1000));
