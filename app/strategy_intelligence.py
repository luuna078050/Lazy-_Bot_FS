...
assumptions. The engine separates short-term execution from higher-timeframe
market regime and never treats an underwater position as an automatic sell.
"""
...
def _rsi(v:Sequence[float],n:int=14)->float:
    ...
    ds=[b-a for a,b in zip(v[-n-1:-1],v[-n:])];g=sum(max(x,0.0) for x in ds)/n;l=sum(max(-x,0.0) for x in ds)/n
...
def _stoch(h:Sequence[float],l:Sequence[float],c:Sequence[float],n:int=14)->tuple[float,float]:
    ...
    def k_at(end:int)->float:
        hh=max(h[end-n+1:end+1]);ll=min(l[end-n+1:end+1]);return 50.0 if hh==ll else (c[end]-ll)/(hh-ll)*100.0
    k=k_at(len(c)-1);ks=[k_at(i) for i in range(max(n-1,len(c)-3),len(c))];return k,(sum(ks)/len(ks) if ks else k)
...
def timeframe_signal(data:dict)->dict:
    ...
    p=c[-1];ma7,ma25,ma99=_ma(c,7),_ma(c,25),_ma(c,99);r=_rsi(c);st_k,st_d=_stoch(h,l,c);sl=_slope(c);atr=_atr_pct(h,l,c)
    ...
    return {"direction":d,"score":round(x,4),"confidence":round(min(1.0,abs(x)+.2),4),"price":p,"ma7":ma7,"ma25":ma25,"ma99":ma99,"rsi14":r,"stoch14_k":st_k,"stoch14_d":st_d,"slope5_pct":sl,"atr14_pct":atr,"ma_spread_pct":ma_spread}
...
def evaluate(timeframes:dict[str,dict],orderbook:dict|None=None)->dict:
    parts={tf:timeframe_signal(timeframes[tf]) for tf in timeframes if tf in TF_WEIGHTS};w=sum(TF_WEIGHTS[t] for t in parts);mtf=sum(parts[t]["score"]*TF_WEIGHTS[t] for t in parts)/w if w else 0.0;micro=sum(parts.get(t,{}).get("score",0.0) for t in EXECUTION_TFS)/max(1,sum(t in parts for t in EXECUTION_TFS));regime=detect_regime(parts);ob=float((orderbook or {}).get("score",0.0));spoof=float((orderbook or {}).get("spoof_risk",0.0))
    final=(mtf*.60+micro*.15+ob*.25) if regime["regime"]=="flat" else (mtf*.55+micro*.25+ob*.20);final=max(-1.0,min(1.0,final))*(1.0-max(0.0,min(1.0,spoof))*.50);d="bullish" if final>.18 else "bearish" if final<-.18 else "neutral"
    return {"direction":d,"score":round(final,4),"mtf_score":round(mtf,4),"micro_score":round(micro,4),"orderbook_score":round(ob,4),"spoof_risk":round(spoof,4),"regime":regime,"timeframes":parts,"execution_horizon":"1-3m","quote_currency":"agnostic"}
