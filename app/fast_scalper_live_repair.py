...
# Reference UI rule: preserve the original interface and its element relationships.
# The only functional UI expansion is six slots -> ten slots.
html=legacy.HTML
...
# Restore the reference control block exactly as a three-part control group:
# BOT BALANCE, WITHDRAW AMOUNT, PROFIT TARGET + Reinvest.
...
new_controls='<div class="card"><div class="row"><div style="flex:1;min-width:210px"><div class="muted" style="font-size:12px;margin:0 0 4px">BOT BALANCE · USDT</div><input id="allocation" class="input" style="width:100%" type="number" step="0.01" min="0" placeholder="150"></div><button type="button" class="btn test" id="allocBtn" style="align-self:end">SET BOT BALANCE</button></div><div class="row" style="margin-top:8px"><div style="flex:1;min-width:210px"><div class="muted" style="font-size:12px;margin:0 0 4px">WITHDRAW AMOUNT · USDT</div><input id="withdrawAmount" class="input" style="width:100%" type="number" step="0.01" min="0" placeholder="0.33"></div><button type="button" class="btn stop" id="withdrawBtn" style="align-self:end">WITHDRAW</button></div><div class="row" style="margin-top:8px;align-items:end"><div style="flex:0 0 180px;max-width:180px"><div class="muted" style="font-size:12px;margin:0 0 4px">PROFIT TARGET · %</div><input id="profit" class="input" style="width:100%" type="number" step="0.01" value="0.31"></div><label style="padding:10px"><input id="reinvest" type="checkbox" checked> Reinvest</label></div><div class="row" style="margin-top:8px"><button type="button" class="btn on" id="onBtn">BOT ON · ACTIVE</button><button type="button" class="btn stop" id="emBtn">EMERGENCY</button><button type="button" class="btn" id="resetBtn">RESET</button><button type="button" class="btn stop" id="offBtn">BOT OFF</button><span class="muted" id="tim">SESSION 00:00 · 24H 00:00</span></div></div>'
html=html.replace(old_controls,new_controls)
html=html.replace("const a=Number($('allocation').value);if(!Number.isFinite(a)||a<=0)throw Error('Enter withdrawal amount');state=await request('/api/withdraw','POST',{amount:a});$('msg').textContent='Withdrawn to Reserve: '+num(a)+' USDT';$('allocation').value='';render()", "const a=Number($('withdrawAmount').value);if(!Number.isFinite(a)||a<=0)throw Error('Enter withdrawal amount');state=await request('/api/withdraw','POST',{amount:a});$('msg').textContent='Withdrawn to Reserve: '+num(a)+' USDT';$('withdrawAmount').value='';render()")
legacy.HTML=html
