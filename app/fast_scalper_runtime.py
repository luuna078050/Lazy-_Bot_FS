...
function renderClosed(a){const total=a.reduce((v,x)=>v+Number(x.pnl||0),0);const cls=total>=0?'pplus':'pminus';$('closedCount').textContent=String(a.length);$('closedSummary').className='summary '+(a.length?cls:'');$('closedSummary').style.color=a.length?(total>=0?'#18c878':'#ef5264'):'';$('closedSummary').innerHTML=a.length?'Closed: '+a.length+' · PnL: '+(total>=0?'+':'')+num(total)+' USDT':'No closed trades'}
