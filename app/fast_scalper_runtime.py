...
def state_payload():
    ...
    for p in s.get('positions',[]):
        ...
        positions.append(q)
    # Canonical derived balances: Account = Reserve + Bot, Free = Bot - invested.
    account=float(s.get('account',0) or 0.0)
    bot=float(s.get('bot',0) or 0.0)
    s['reserve']=max(0.0,account-bot)
    s['free']=max(0.0,bot-legacy.invested())
...
function renderClosed(a){const total=a.reduce((v,x)=>v+Number(x.pnl||0),0);const cls=total>=0?'pplus':'pminus';$('closedCount').textContent=String(a.length);$('closedSummary').className='summary '+cls;$('closedSummary').style.removeProperty('color');$('closedSummary').innerHTML=a.length?'Closed: '+a.length+' · PnL: '+(total>=0?'+':'')+num(total)+' USDT':'No closed trades'}
...
async def allocation_core(b:dict):
    ...
    if legacy.S.get('mode')=='PAPER':
        ...
        # Allocation is an internal transfer between Reserve and Bot.
        # Account Balance is total capital and MUST NOT change here.
        legacy.S['bot']=amount
        legacy.S['free']=max(0.0,amount-legacy.invested())
        legacy.refresh_reserve()
        assert abs(float(legacy.S['account'])-(float(legacy.S['bot'])+float(legacy.S['reserve']))) < 1e-6
        return state_payload()
