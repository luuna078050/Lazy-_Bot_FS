...
async def manage_fixed2():
    now=time.time()
    timeout_age=float(getattr(legacy,'MAX_AGE',60) or 60)
...
# 3) AUTO TOP-10: typed JSON body and safe rotation. Old position is closed before slot changes.
for r in list(legacy.app.router.routes):
...
async def auto_top10_fixed2(b:AutoTop10Body):
    ...
    for x in legacy.S.get('ranking',[]):
        ...
        if s and s not in seen and s.endswith('USDT') and len(s)>4:
...
# 4) Restore compact typography without changing the approved layout/controls.
html=legacy.HTML
...
if 'data-final-compact-ui' not in html:
    html=html.replace('</head>',compact+'</head>',1)
legacy.HTML=html
