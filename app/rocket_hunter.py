...
def classify_candles(candles:list[dict])->dict:
    """Classify the last 3m bars: IGNITION, RELOAD, ORBIT or WAIT."""
    if len(candles)<8:return {"state":"WAIT","score":0.0,"entry_ready":False,"reason":"not_enough_bars"}
    c=[float(x["close"]) for x in candles];v=[float(x.get("volume",0)) for x in candles];h=[float(x["high"]) for x in candles];l=[float(x["low"]) for x in candles]
    last=c[-1];short_move=(last/c[-4]-1)*100 if c[-4] else 0;prev_move=(c[-4]/c[-8]-1)*100 if c[-8] else 0;base=mean(v[-8:-3]) or 1;vr=v[-1]/base;breakout=last>max(h[-6:-1]);pullback=c[-2]<max(c[-6:-2]) and last>c[-2];accelerating=short_move>.8 and short_move>prev_move*.65
