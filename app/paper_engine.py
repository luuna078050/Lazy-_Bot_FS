...
def _close(symbol,pos,last,reason):
 ...
 _state['trades'].append({'symbol':symbol,'side':'SELL','entry_price':pos['entry_price'],'exit_price':last,'amount':pos['amount'],'gross_pnl':gross,'fee':fee,'net_pnl':net,'reason':reason,'opened_at':pos['opened_at'],'closed_at':_now(),'fills':list(pos.get('fills',[])),'fill_count':len(pos.get('fills',[]))})
...
def _tick(symbol,allocation,target,minp,sl,maxhold,gateway):
 try:
  ...
  with _lock:
   ...
   if pos:
    gross=(last-pos['entry_price'])*pos['amount']; fee=(pos['entry_price']*pos['amount']+last*pos['amount'])*pos['fee_pct']/100; net=gross-fee; age=time.time()-pos['opened_ts']; reason=None
    if net>=target:reason='TARGET'
    elif sl>0 and (last/pos['entry_price']-1)*100<=-sl:reason='SL'
    elif age>=maxhold:reason='TIMEOUT' if net>=minp else 'CRITICAL_EXIT'
   ...
   if order:
    ...
    if chunk<=0:return
    cost=chunk*last; _state['balance']-=cost; order['fills'].append({'time':_now(),'amount':chunk,'price':last,'cost':cost}); order['filled_amount']+=chunk; order['remaining_amount']-=chunk
    order['status']='FILLED' if order['remaining_amount']<=order['requested_amount']*.00001 else 'PARTIALLY_FILLED'
    if order['status']=='FILLED':
     avg=sum(f['amount']*f['price'] for f in order['fills'])/order['filled_amount']; _state['open_positions'][symbol]={'symbol':symbol,'entry_price':avg,'amount':order['filled_amount'],'allocated_usdt':sum(f['cost'] for f in order['fills']),'opened_at':order['fills'][0]['time'],'opened_ts':time.time(),'fee_pct':_state['config']['fee_pct'],'signal_24h_pct':pct,'fills':list(order['fills'])}
    return
   if pct<0.15 or _state['balance']<=0 or allocation<=0:return
   amt=allocation/last; _state['orders'][symbol]={'symbol':symbol,'side':'BUY','requested_usdt':allocation,'requested_amount':amt,'filled_amount':0.0,'remaining_amount':amt,'status':'NEW','fills':[]}
...
def _loop(gateway):
 while not _stop.is_set():
  ...
  for i,s in enumerate(cfg.get('pairs',[])):
   ...
   a=cfg['initial_balance']*cfg['allocations'][i]/100 if i<len(cfg['allocations']) else 0
   _tick(s,a,cfg['target_usdt'],cfg['min_usdt'],cfg['sl_pct'],cfg['max_hold'],gateway)
  time.sleep(5)
 with _lock:_state['running']=False;_state['stopped_at']=_now()
def start_paper(config,gateway):
 global _thread
 pairs=[str(x).strip().upper() for x in config.get('pairs',[]) if str(x).strip()]; alloc=[float(x) for x in config.get('allocations',[])] ; capital=float(config.get('capital',0))
