...
# Requested Fast Scalper analysis set:
# 1s + derived 30s + Binance 1m/3m/5m/15m/30m candles.
TIMEFRAMES = ("1s", "1m", "3m", "5m", "15m", "30m")
DETAIL_SYMBOL_LIMIT = 80  # 80*6 kline streams + 80 aggTrade streams stays below Binance's 1024-stream limit.
...
def _promote_streams(self):
    ...
    while not self._stop.is_set() and time.time() < deadline:
        ...
        if len(symbols) >= 50:
            ...
            print(f"RADAR_DETAIL_STREAMS_READY {len(symbols)} timeframes={','.join(TIMEFRAMES)} plus_30s_derived", flush=True)
            try:
                if self._ws:
                    self._ws.close()
            except Exception:
...
def _metrics(self, symbol, price):
    ...
    if pulse:
        latest = pulse[-1]; target_sec = latest["sec"] - 30
        base = min(pulse, key=lambda x: abs(x["sec"] - target_sec))
        if base.get("price", 0) > 0:
            p30 = (latest["price"] / base["price"] - 1) * 100
        recent = pulse[-30:]
        pvol = sum(x["quote"] for x in recent) / max(1, len(recent))
        br = latest["buy_quote"] / latest["quote"] if latest["quote"] > 0 else .5
    recent_1m = bars["1m"][-8:]; ranges = []; vols = []
    for b in recent_1m:
    ...
    return {"change_1s_pct": changes["1s"], "change_30s_pct": p30, "change_1m_pct": changes["1m"], "change_3m_pct": changes["3m"], "change_5m_pct": changes["5m"], "change_15m_pct": changes["15m"], "change_30m_pct": changes["30m"], "risk_pct": risk, "volume_ratio": vr, "pulse_volume": pvol, "buy_ratio": br}


def _pulse_score(self, m):
    return min(1, max(0, .20 * min(1, max(0, m["change_1s_pct"]) / .08) + .25 * min(1, max(0, m["change_30s_pct"]) / .20) + .25 * min(1, max(0, m["change_1m_pct"]) / .40) + .15 * min(1, max(0, m["volume_ratio"] - 1) / 3) + .15 * min(1, max(0, (m["buy_ratio"] - .5) / .30))))


def _score(self, t, m):
    q = max(1, float(t.get("q", 0) or 0)); liq = min(1, max(0, math.log10(q) / 9))
    multi = .20*m["change_1s_pct"] + .25*m["change_30s_pct"] + .20*m["change_1m_pct"] + .15*m["change_3m_pct"] + .10*m["change_5m_pct"] + .07*m["change_15m_pct"] + .03*m["change_30m_pct"]
    mom = min(1, max(0, multi / 1.2)); act = min(1, max(0, m["volume_ratio"] - 1) / 3); pulse = self._pulse_score(m); pen = min(.45, max(0, m["risk_pct"]) / 4)
...
def snapshot(self, limit=FINAL):
    ...
    s1=[x for x in s1 if x[2]["volume_ratio"]>=.8 or x[2]["change_30s_pct"]>.08]; s2=s1[:STAGE2]; self.stage_counts["stage2"]=len(s2)
    s2=[x for x in s2 if x[2]["change_3m_pct"]>-.35 and x[2]["change_5m_pct"]>-.80]; s3=s2[:STAGE3]; self.stage_counts["stage3"]=len(s3); s3.sort(key=lambda x:x[3],reverse=True); final=s3[:FINAL]; self.stage_counts["top15"]=len(final)
    out=[]
    for s,d,m,score in final:
        p=float(d.get("c",0) or 0); sig="BUY" if m["change_30s_pct"]>0 and m["change_1m_pct"]>0 and m["change_3m_pct"]>0 else ("WATCH" if m["change_1m_pct"]>0 else "WAIT")
        target=min(.012,max(.0035,abs(m["change_1m_pct"])/100*.8))
        out.append({"symbol":s[:-4]+"/USDT","price":p,"change_24h_pct":round((p/float(d.get("o",p) or p)-1)*100,3),"quote_volume_24h":float(d.get("q",0) or 0),"score":round(score,2),"signal":sig,"estimated_entry":p,"estimated_exit":p*(1+target),"estimated_stop":p*(1-.004),"change_1s_pct":round(m["change_1s_pct"],4),"change_30s_pct":round(m["change_30s_pct"],4),"change_1m_pct":round(m["change_1m_pct"],4),"change_3m_pct":round(m["change_3m_pct"],4),"change_5m_pct":round(m["change_5m_pct"],4),"change_15m_pct":round(m["change_15m_pct"],4),"change_30m_pct":round(m["change_30m_pct"],4),"volume_ratio":round(m["volume_ratio"],2),"pump_events":0,"pump_score":round(self._pulse_score(m),3),"hold_seconds":90,"analysis_timeframes":["1s","30s","1m","3m","5m","15m","30m"]})
...
print("RADAR_TIMEFRAMES_ENABLED 1s,30s,1m,3m,5m,15m,30m HOLD=90s",flush=True)
